...
/** Persistent Mattermost server configuration record. */
@Entity
@Table(name = "mattermost_servers")
data class MattermostServer(
    @Id @UuidGenerator(style = UuidGenerator.Style.VERSION_7) val id: UUID = UUID(0, 0),
    @Column(nullable = false, unique = true, length = 100) val name: String = "",
    @Column(nullable = false, length = 500) val baseUrl: String = "",
    @Convert(converter = SecretRefConverter::class)
    @Column(nullable = false, length = 500)
    val token: SecretRef = SecretRef.literal(""),
