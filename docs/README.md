...
Start with [What is Streampack?](explanation/what-is-streampack.md) if you are orienting yourself, [Deploy `server-streampack`](tutorials/deploy-server-streampack.md) if you are putting the bundled server on a machine, or [Run the infobot on IRC](tutorials/run-infobot-on-irc.md) if you want the protocol path first.
